# 1 "Network_Optimization.c"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "Network_Optimization.c"
# 11 "Network_Optimization.c"
# 1 "Network_Optimization.h" 1
...
# 19 "Source_Header.h" 2
# 9 "Network_Optimization.h" 2
# 1 "Graph.c" 1
# 1 "Graph.h" 1
...
struct Graph* constructGraph() {
 ...
 while (i < g->totalVertices) {
  g->list[i].vertex = i;
  g->list[i].weight = 2147483647;
  g->list[i].next = ((void *)0);
  i++;
 }
...
void initialze_random_array(int source_vertex, int target_vertex) {
 if (random_array != ((void *)0)) {
  free(random_array);
 }

 random_array = malloc(sizeof(int) * 10);
 int i = 0;
 while (i < 10) {
  random_array[i] = i++;
 }
 swap_array_elements(random_array, source_vertex, 10 - 1);
 swap_array_elements(random_array, target_vertex, 10 - 2);
 random_array_size = 10 - 2;
}
...
# 11 "Network_Optimization.h" 2
# 1 "Max_Capacity_Dijkstra_Without_Heap.c" 1
# 1 "Max_Capacity_Dijkstra_Without_Heap.h" 1
# 9 "Max_Capacity_Dijkstra_Without_Heap.h"
# 1 "Graph.c" 1
# 1 "Graph.h" 1
...
# 10 "Max_Capacity_Dijkstra_Without_Heap.h" 2
# 1 "Linked_List.c" 1
# 1 "Linked_List.h" 1
# 2 "Linked_List.c" 2
...
# 11 "Max_Capacity_Dijkstra_Without_Heap.h" 2
# 1 "Random_Vertex.c" 1
# 1 "Random_Vertex.h" 1
...
# 12 "Max_Capacity_Dijkstra_Without_Heap.h" 2
# 20 "Max_Capacity_Dijkstra_Without_Heap.h"
...
void dijsktra(struct Graph *graph, struct Dijkstra_Arrays* results,
 ...
 struct List_Node * max_fringe_node = ((void *)0);
 int max_fringe_vertex, edge_vertex;
 while (fringe_list) {
  max_fringe_node = get_max_fringe(&fringe_list);
  max_fringe_vertex = max_fringe_node->vertex;
  status[max_fringe_vertex] = 2;
  edges = graph->list[max_fringe_vertex].next;

  while (edges) {
   edge_vertex = edges->vertex;
   if (status[edge_vertex] == 0) {
    status[edge_vertex] = 1;
    dad[edge_vertex] = max_fringe_vertex;
    dist[edge_vertex] = (((dist[max_fringe_vertex])< (edges->weight))?(dist[max_fringe_vertex]):(edges->weight));
    insert_list(&fringe_list, edge_vertex, dist[edge_vertex]);
   } else if (status[edge_vertex] == 1
     && dist[edge_vertex]
       < (((dist[max_fringe_vertex])< (edges->weight))?(dist[max_fringe_vertex]):(edges->weight))) {
    dist[edge_vertex] = (((dist[max_fringe_vertex])< (edges->weight))?(dist[max_fringe_vertex]):(edges->weight));
    dad[edge_vertex] = max_fringe_vertex;
...
# 12 "Network_Optimization.h" 2
# 20 "Network_Optimization.h"
...
void link_creation(struct Graph* graph, int source_index, int target_index) {
 ...
 if (!child_vertex) {
  child_vertex = graph->list[source_index].next;
  child_vertex->vertex = target_index;
 }
}
...
int main(void) {
 ...
 cpu_time = ((double) (clock() - start)) / 1000000l;
 printf("\n\nTotal Time Taken: %f\n\n", cpu_time);
 return 0;
}
